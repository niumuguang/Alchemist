-- ------------------------------------------------------------------------------
-- 1.分钟表触发器 用于更新前的函数出发， 主要进行计算各种技术指标
-- 2.需要修改触发器名称 i1509_5Min_BUPD
-- 4.需要修改触发器 触发表名称 i1509_5Min
-- 5.需要全部替换表名称 i1509_5Min
-- ------------------------------------------------------------------------------
USE `IronDB`;
DELIMITER $$
CREATE DEFINER=`root`@`localhost` TRIGGER `i1509_5Min_BUPD` BEFORE UPDATE ON `i1509_5Min`
FOR EACH ROW 

begin

declare tbName varchar(30) default "IronDB.i1509_5Min";
...
-- 只有更新触发器 时比较最高价和最低价
#获取最高价和最低价
SELECT HighestPrice into mHighestPrice FROM IronDB.i1509_5Min ORDER BY id DESC LIMIT 0,1;
SELECT LowestPrice into mLowestPrice FROM IronDB.i1509_5Min ORDER BY id DESC LIMIT 0,1;
#最高价
...
# MA3
while i<=2 do
SELECT ClosePrice into tmpP FROM IronDB.i1509_5Min ORDER BY id DESC LIMIT i,1;
...
set stdPower = stdPower + power((tmpP-stdMa),2);
set i = i+1;
end while;
set stdTmp = stdPower/26;
set stdTmp = sqrt(stdTmp);
set new.BollUp = stdMa + 2*stdTmp;
set new.BollDown = stdMa - 2*stdTmp;
