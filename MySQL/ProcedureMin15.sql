...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert15Min_ag1506`(in InstrumentID varchar(10),
in TradingDay date,
in UpdateTime time,
in LastPrice float,
in AskPrice1 float,
...
DECLARE MinConst int default 15;
#####################################################################################################
#变量初始化
...
SELECT count(id) into MinTableCount  FROM IronDB.ag1506_15Min;
select minute(mUpdateTime) into MinCompareTmp;
if (MinTableCount = 0) && (MOD(MinCompareTmp, MinConst)=0 ) then
	if mAskPrice1 != 0 then
	insert into ag1506_15Min(
	InstrumentID,
	TradingDay,
	UpdateTime,
	MinuteNum,
...
#分钟表根据需要进行修改ag1506_15Min  HourCompare1
select MinuteNum into MinCompare1 from IronDB.ag1506_15Min order by id desc limit 0,1;
select UpdateTime into CompareTempTime from IronDB.ag1506_15Min order by id desc limit 0,1;
select TradingDay into CompareTempDate from IronDB.ag1506_15Min order by id desc limit 0,1;
...
SELECT MySQLTimeStamp into CompareTempDateTime2 from IronDB.ag1506_15Min order by id desc limit 0,1;
set @res = ABS( TIMESTAMPDIFF(MINUTE, CompareTempDateTime2, CompareTempDateTime1) );
#select TimeMachine(1,1,1, 2,2,2) into @resBool;
#select Hour(mUpdateTime) into HourCompare2;
#MOD(MinCompare1, 5)
	if (@res<MinConst) then
	#先从最后一行获取需要的数据
	#分钟表根据需要进行修改
	#MinuteNum, AskPrice1, BidPrice1, AskVolume1, BidVolume1
	#AveragePrice, HighestPrice, LowestPrice, Volume, OpenInterest
	SELECT SubDataNum into tmpSubDataNum FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT AveragePrice into tmpAveragePrice FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT HighestPrice into tmpHighestPrice FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT LowestPrice into tmpLowestPrice FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT StartVolume into tmpStartVolume FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT OpenInterest into tmpOpenInterest FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT AskPrice1 into tmpAskPrice1 FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT BidPrice1 into tmpBidPrice1 FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT AskVolume1 into tmpAskVolume1 FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	SELECT BidVolume1 into tmpBidVolume1 FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	#SELECT MinuteNum,SubDataNum into tmpMinuteNum,tmpSubDataNum FROM IronDB.ag1506_15Min ORDER BY id DESC LIMIT 1;
	#tmpAskVolume1
	#计算需要计算的数据
...
#如果分钟数不相等 则认为新开始了一分钟 插入新的数据
		...
		if (MOD(MinCompareTmp,MinConst)=0) then
		insert into ag1506_15Min(
		InstrumentID,
		TradingDay,
		UpdateTime,
